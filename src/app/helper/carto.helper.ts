...
import { BackendApiService } from "../data/services/backend-api.service";
/**
...
export interface layersInMap {
   ...
   */
  descriptionSheetCapabilities: "osm" | "querry" | "sigfile";
}
...
export const MGEXTENT = [31.310439121, -26.831521626, 61.733870176, -9.985838040]
/**
...
export class CartoHelper {
   ...
   * @returns ImageLayer
   */
  constructShadowLayer(
    featureToShadow: Feature<Geometry>[]
  ): ImageLayer<ImageSource> {
    var worldGeojson = {
  ...
  constructLayer(
    couche: DataOSMLayer
  ): TileLayer<TileSource> | LayerGroup | VectorLayer<VectorSource<Geometry>> | ImageLayer<ImageSource> {
    let layer:
      | TileLayer<TileSource>
      | ImageLayer<ImageSource>
      | LayerGroup
    ...
    if (couche.type == "xyz") {
      layer = new TileLayer({
        source: new XYZ({
          url: couche.url,
          attributions:
            '<a target="_blank" href="https://www.openstreetmap.org/copyright"> © OpenStreetMap </a> contributors , develop by <a target="_blank" href="https://twitter.com/armeltayou"> @armeltayou </a>',
        }),
        /**
         * so that map.forEachLayerAtPixel work as expected
         * @see https://openlayers.org/en/latest/apidoc/module-ol_PluggableMap-PluggableMap.html#forEachLayerAtPixel
      ...
      layer = new ImageLayer({
         ...
         */
        className: couche.nom + "___" + couche.type_layer,
        maxResolution: this.map.getView().getResolutionForZoom(5),
      });
    ...
    //         couche.url +
    //         "?bbox=" +
    //         transformExtent(extent_vieuw, "EPSG:3857", "EPSG:4326").join(",") +
    //         "&SERVICE=WFS&VERSION=1.1.0&REQUEST=GETFEATURE&outputFormat=GeoJSON&typeName=" +
    //         couche.identifiant.join(",");
  ...
  setPropertiesToLayer(layer: any, couche: DataOSMLayer) {
    if (layer instanceof LayerGroup) {
      for (
        ...
        element.set(
          "identifiant",
          couche.identifiant ? couche.identifiant.join(",") : undefined
        );
   ...
   */
  setLayerInvisible(
    layer: VectorLayer<VectorSource<Geometry>> | ImageLayer<ImageSource>
  ) {
   ...
   * @return Array<any>
   */
  getLayerByProperties(
    properties: Object,
    isLayerGroup: boolean = false
  ): Array<any> {
    var layer_to_remove = [];
    ...
    for (let index = 0; index < all_layers.length; index++) {
      ...
      for (const key in properties) {
        if (
          ...
          layer.get("properties")[key] != undefined
        ) {
  ...
  getLayerByPropertiesCatalogueGeosm(properties: {
    ...
    this.getAllLAyerInMap().forEach((layer) => {});
  ...
  getAllLayersInToc(): Array<layersInMap> {
    ...
    let layerOfSameGroup = cloneAllLayers.find((nnn, index) => {
      return cloneAllLayers.find(
        (x, ind) =>
          x.properties.couche_id + x.properties["type"] ===
            nnn.properties.couche_id + nnn.properties["type"] && index !== ind
      );
    });
    while (layerOfSameGroup) {
      let allInstanceDuplicates = reponseLayers.filter(
        (t) =>
          t.properties.couche_id + t.properties["type"] ===
          layerOfSameGroup.properties.couche_id +
            layerOfSameGroup.properties["type"]
      );
      allInstanceDuplicates.map((t) => duplicates.push(t));
      let allLayersDuplicate: Array<BaseLayer> = flatDeep(
        allInstanceDuplicates.map((h) => h.layer)
      );
      let clone = { ...layerOfSameGroup };
      clone.layer = allLayersDuplicate;
      duplicatesGrouped.push(clone);

      let duplicatesKey = duplicates.map(
        (y) => y.properties.couche_id + y.properties["type"]
      );
      cloneAllLayers = cloneAllLayers.filter(
        (g) =>
          !duplicatesKey.includes(g.properties.couche_id + g.properties["type"])
      );
  ...
  ): Array<ImageLayer<ImageSource> | TileLayer<TileSource>> {
    var layers: Array<ImageLayer<ImageSource> | TileLayer<TileSource>> = [];
  ...
  mapHasCliked(
    ...
    callback: (param: dataFromClickOnMapInterface) => void
  ) {
   ...
   */
  getFeatureInfoFromWmsSource(
    source: ImageWMS,
    coordinates: Array<number>
  ): string {
    var viewResolution = this.map.getView().getResolution();
    var url =
      source.getFeatureInfoUrl(coordinates, viewResolution, "EPSG:3857", {}) +
      "&WITH_GEOMETRY=true&INFO_FORMAT=application/json&FI_LINE_TOLERANCE=17&FI_POLYGON_TOLERANCE=17&FI_POINT_TOLERANCE=17";
    return url;
  }
